...
def filter_gauss(datapoint, srange=None):
    if srange is None:
        srange = [1, 1]
    img, label = datapoint
    imgn = np.transpose(img.numpy(), (1, 2, 0))
...
def noise_uniform(datapoint):
    ...
    if imgn.shape[2] != 1:
        ...
        imgn_random = np.float32(np.random.uniform(size=imgn.shape))
        assert torch.from_numpy(np.transpose(imgn_random, (
            2, 0, 1))).shape == img.shape, 'torch.from_numpy(np.transpose(imgn_random, (2,0,1))).shape wrong: ' + str(
            torch.from_numpy(np.transpose(imgn_random, (2, 0, 1))).shape)
        return torch.from_numpy(np.transpose(imgn_random, (2, 0, 1))), label
...
class monochrome:
    def __init__(self, color):
        super().__init__()
        self.color = color
        self.__name__ = 'monochrome' + str(color)
...
class lh_ratios_style:
    def __init__(self, mu):
        super().__init__()
        self.mu = mu
        self.__name__ = 'lh_ratios_style' + str(mu)

    def __call__(self, datapoint):
        img, label = datapoint
        mask = torch.rand_like(img) < self.mu
        noise = torch.rand_like(img)
        lh_rat_background_img = (~mask) * img + mask * noise
        return lh_rat_background_img, label
